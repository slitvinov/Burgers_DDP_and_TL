...
kx_gamma = (2 * math.pi / Lx) * np.concatenate(
    (np.arange(0, (NY / gamma) / 2 + 1,
               dtype=float), np.arange(
                   (-(NY / gamma) / 2 + 1), 0, dtype=float)))
kx_bar = (2 * math.pi / Lx) * np.concatenate((np.arange(
    0, NY / 2 + 1, dtype=float), np.arange((-NY / 2 + 1), 0, dtype=float)))
kx_prime = (2 * math.pi / Lx) * np.concatenate((np.arange(
    0, NX / 2 + 1, dtype=float), np.arange((-NX / 2 + 1), 0, dtype=float)))
...
def calc_cp(u, gamma):
	...
	M = ((delta**2) * filter_bar(np.linalg.norm(der_u, 2) * der_u, gamma) -
	     (delta_sub**2) * np.linalg.norm(der_u_sub, 2) * der_u_sub).squeeze()
...
sio.savemat('./DSMAG_region_.mat', {'u_pred': u_store, 'sub_pred': sub_store})
