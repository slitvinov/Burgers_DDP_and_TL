...
def shift_data(data1, data2):
	...
	for i in range(data1.shape[0]):
		data1[i, :] = np.concatenate((data1[i,
		                                    shifts[i]:], data1[i, :shifts[i]]))
		data2[i, :] = np.concatenate((data2[i,
		                                    shifts[i]:], data2[i, :shifts[i]]))
...
full_input[:train_region, :], full_output[:train_region, :] = shift_data(
    full_input[:train_region, :], full_output[:train_region, :])

norm_input, mean_input, std_input = normalize_data(
    full_input[:train_region, :])

norm_output, mean_output, std_output = normalize_data(
...
kx = (2 * math.pi / Lx) * np.concatenate((np.arange(
    0, NX / 2 + 1, dtype=float), np.arange(
        (-NX / 2 + 1), 0, dtype=float))).reshape([NX, 1])
...
D2_tensor = np.float32(
    (D2[0:int(NX / 2)] -
     np.mean(D2[0:int(NX / 2)]) / np.std(D2[0:int(NX / 2)])))
...
force_bar = force_dict['f_bar'][:,
                                int((reg - 1) * 12500) + int(pred_start / s):]

u_old = full_input[pred_start - 1, :].reshape([NX, 1])
u = full_input[pred_start, :].reshape([NX, 1])
u_fft = fft(u, axis=0)
u_old_fft = fft(u_old, axis=0)
subgrid_prev_n = model.predict(((u_old - mean_input) / std_input).reshape(
    (1, 128))).reshape(128, 1)
...
sio.savemat(
    './DDP_results_trained_' + str(int(train_num / 1000)) + '_region_' +
    region + '_new.mat', {
        'u_pred': u_store,
        'sub_pred': sub_store
    })
